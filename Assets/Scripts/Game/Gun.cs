...
public class Gun : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (mFiring && fireEnabled)
        {
            ...
            if(shotgun){
                secondsPerBullet = 5.0f / (fireRate / 60.0f);
            }else{
    ...
    public void ShootGun(Transform director)
    {
         ...
         */
        if(shotgun){
            float bulletAngle = shotgunSpread/shotgunBullets - 1;
            float bulletRotation = - shotgunSpread / 2;
            for(float bullet = 0f ; bullet < shotgunBullets ; bullet++){
                SpawnBullet(director.position, Quaternion.Euler(director.eulerAngles) * Quaternion.Euler(0f, 0f, bulletRotation));
                bulletRotation = bulletRotation + bulletAngle;
            }
        }else{
